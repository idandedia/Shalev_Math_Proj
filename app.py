...
st.set_page_config(page_title="אלופי החשבון - כיתה ב'", page_icon="🧮", layout="centered")
...
st.markdown(
    ...
    .app-shell {
        ...
        box-shadow: 0 14px 30px rgba(38, 85, 145, 0.14);
    }
    .main-title {
        font-size: 2.3rem;
        font-weight: 800;
        ...
        <script>
            ...
            if (!window[key]) {{
                ...
                if (AudioContextClass) {{
                    ...
                    const baseDur = soundType === "success" ? 0.13 : 0.16;
                    pattern.forEach((freq, i) => {{
                        const osc = ctx.createOscillator();
                        const gain = ctx.createGain();
                        osc.type = soundType === "success" ? "triangle" : "sine";
                        osc.frequency.value = freq;
                        gain.gain.setValueAtTime(0.0001, ctx.currentTime + i * baseDur);
                        gain.gain.exponentialRampToValueAtTime(0.18, ctx.currentTime + i * baseDur + 0.02);
...
st.markdown('<div class="main-title">🧮 אלופי החשבון - כיתה ב\'</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">חיבור וחיסור עד 100 • 20 שלבים של כיף ותרגול</div>', unsafe_allow_html=True)
...
if st.session_state.feedback_text:
    if st.session_state.feedback_type == "success":
        st.markdown(
            f'<div class="success-card">{st.session_state.feedback_text}</div>',
            unsafe_allow_html=True,
        )
